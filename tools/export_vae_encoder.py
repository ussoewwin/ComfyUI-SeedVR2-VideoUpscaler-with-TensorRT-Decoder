...
def main() -> None:
    ...
    encoder = Encoder(vae).eval().to(device="cuda", dtype=torch.float16)
    dummy_video = torch.zeros((1, args.channels, args.frames, args.height, args.width), dtype=torch.float16, device="cuda")
    args.output.parent.mkdir(parents=True, exist_ok=True)
    export_portable_onnx(encoder, (dummy_video,), args.output, legacy=args.legacy_export)
    print(f"Exported VAE encoder ONNX to {args.output}")
