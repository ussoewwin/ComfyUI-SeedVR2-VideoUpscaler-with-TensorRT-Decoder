...
def main() -> int:
    ...
    parser.add_argument(
        "--tc_forward",
        action="store_true",
        help="Apply HSWQ Tensor Core forward path for NVFP4 (Option C: pooled CUDA quantize + cuBLAS scaled_mm_nvfp4). No ConvRot.",
    )
    parser.add_argument(
        "--hswq_path",
        default=None,
    ...
    # --- Option C: Apply HSWQ TC forward patch if requested ---
    if args.tc_forward:
        # Auto-detect HSWQ nodes path if not provided
        hswq_nodes = args.hswq_path
        if hswq_nodes is None:
            hswq_candidate = os.path.join(comfy_root, "custom_nodes",
                                          "ComfyUI-HSWQ-Loader-and-Tools", "nodes")
            if os.path.isdir(hswq_candidate):
                hswq_nodes = hswq_candidate
            else:
                raise RuntimeError(
                    "--tc_forward requires ComfyUI-HSWQ-Loader-and-Tools. "
                    "Pass --hswq_path /path/to/nodes"
                )
        print(f"[BENCH] Applying HSWQ TC forward patch (Option C)...")
        print(f"[BENCH] HSWQ nodes: {hswq_nodes}")
        ...
        print("[BENCH] TC forward patch applied — NVFP4 branch will use pooled CUDA quantize + cuBLAS scaled_mm_nvfp4")
    ...
    nvfp4_label = "NVFP4 + TC forward" if args.tc_forward else "NVFP4 (native QuantizedTensor)"
    ...
    out_nvfp4_name = "seedvr2_nvfp4_tc.png" if args.tc_forward else "seedvr2_nvfp4.png"
    out_nvfp4 = Path(args.output_dir) / out_nvfp4_name
    ...
    out_diff_name = "seedvr2_diff_tc.png" if args.tc_forward else "seedvr2_diff.png"
    out_diff = Path(args.output_dir) / out_diff_name
    diff.save(out_diff)

    tc_tag = " + TC forward" if args.tc_forward else ""
    print(f"\n=== Results (FP16 vs native NVFP4{tc_tag}, same videoupscaler pipeline) ===")
    print(f"  MSE:  {mse:.6f}")
