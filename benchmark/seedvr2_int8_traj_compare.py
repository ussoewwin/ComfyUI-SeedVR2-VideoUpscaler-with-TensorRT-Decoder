#!/usr/bin/env python3
"""
SeedVR2 INT8 deterministic trajectory-divergence comparator (FP16 vs ConvRot INT8)
==================================================================================
SeedVR2 版の決定論的・多シード軌跡比較ベンチマーク(HSWQ INT8 専用)。

hswq の ``benchmark/zi_convrot_nvfp4_traj_compare.py``(Z Image 向け)を
SeedVR2 videoupscaler 向けに移植したもの。既存の画像比較ベンチ
(``seedvr2_int8_bench.py``)の「画像を入力 → 加工画像(超解像結果)を
MSE / SSIM / diff PNG で比較する」ロジックはそのまま維持し、
それ以外の機能(下記)を取り入れている。

※ ConvRot INT8 と ConvRot NVFP4 は完全に別物のため、スクリプトも分離している。
   本スクリプトは HSWQ INT8(int8_tensorwise, optional ConvRot)専用。
   NVFP4 用は ``seedvr2_nvfp4_traj_compare.py``(混ぜるな)。

取り入れた機能(参照: zi_convrot_nvfp4_traj_compare.py)
--------------------------------------------------------
1. per-step latent 軌跡比較(EulerSampler にフックし、毎ステップの
   noisy latent x とモデル予測 x0 の cosine / MSE を FP16 vs INT8 で比較)
...
5. GEMM モード要約(HSWQ INT8 ネイティブ int8_tensorwise パスの報告)。
6. 決定論的比較のための cuDNN deterministic 固定。

量化チェックポイントは HSWQ INT8(int8_tensorwise, optional ConvRot)専用。
comfy_quant マーカーをスキャンして INT8 レイヤー数と convrot レイヤー数を
報告する。

Usage:
    python benchmark/seedvr2_int8_traj_compare.py ^
        --fp16  seedvr2_ema_7b_fp16.safetensors ^
        --int8  seedvr2_7b_int8_convrot.safetensors ^
        --vae   ema_vae_fp16.safetensors ^
        [--image input.png] [--seeds "42,1337,7"] [--steps 50] [--cfg 7.5]
"""
...
def count_int8_convrot_markers(path: str) -> tuple[int, int]:
    """
    Scan safetensors .comfy_quant markers for INT8 (int8_tensorwise) and
    convrot flags. Returns (n_convrot, n_int8_layers).

    ConvRot detection matches HSWQ's ``_comfy_quant_conf_has_convrot``:
    either ``conf["convrot"]`` directly or ``conf["params"]["convrot"]``.
    """
        ...
        with safe_open(path, framework="pt", device="cpu") as f:
            for key in f.keys():
                ...
                if fmt != "int8_tensorwise":
                    continue
                n_quant += 1
                cr = conf.get("convrot") is True or str(
                    conf.get("convrot", "")
                ).lower() in ("1", "true")
                params = conf.get("params")
                if isinstance(params, dict):
                    cr = cr or params.get("convrot") is True
                if cr:
                    n_cr += 1
    except Exception as ex:
        print(f"  [Note] int8 marker scan failed for {path}: {ex}")
        return 0, 0
    return n_cr, n_quant
...
def print_int8_gemm_summary() -> None:
    """Print the INT8 GEMM path (always native HSWQ int8_tensorwise)."""
    print("\n" + "-" * 72)
    print("[HSWQ QUANT] GEMM MODE: native INT8 (int8_tensorwise, dequant GEMM)")
    print("  (INT8 forward statistics are not exposed by the HSWQ nodes package)")
    print("-" * 72)
...
def parse_args():
    ap = argparse.ArgumentParser(
        description=(
            "SeedVR2 INT8 deterministic per-step trajectory divergence "
            "(FP16 vs ConvRot INT8), image-in / processed-image-out"
        )
    )
    ap.add_argument("--fp16", required=True, help="BF16/FP16 baseline model path")
    ap.add_argument(
        "--int8", dest="int8_path", required=True,
        help="HSWQ INT8 (int8_tensorwise) quantized model path (convrot optional)",
    )
...
def main() -> int:
    ...
    int8_path = _resolve_weight(args.int8_path, model_dir_path, "--int8")
    ...
    # --- quant checkpoint validation (INT8) ---
    ...
    n_cr, n_quant = count_int8_convrot_markers(str(int8_path))
    print("[BENCH] quant kind: int8 (dedicated INT8 trajectory bench)")
    print(f"[CONVROT markers] int8_layers={n_quant} convrot_stamps={n_cr}")
    ...
    print("[BENCH] mode: INT8 (construction-time mixed_precision_ops)")
    ...
    quant_tag = "int8"
    quant_label = "INT8 (native QuantizedTensor)"
